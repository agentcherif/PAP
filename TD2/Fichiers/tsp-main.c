...
void verifier_minimum(int lg, chemin_t chemin)
{
  if (lg + distance[0][chemin[nbVilles - 1]] < minimum){
    ...
    if (lg + distance[0][chemin[nbVilles - 1]] < minimum)
    {
      minimum = lg + distance[0][chemin[nbVilles - 1]];//ma distance
      printf("%3d :", minimum);
...
void tsp_ompfor(int etape, int lg, chemin_t chemin, int mask)
{
  ...
  if (etape > grain)
    ...
    {
      ici = chemin[etape - 1];
      #pragma omp parallel for if (etape <= grain) firstprivate(ici,dist) num_threads(nbVilles-etape) schedule(dynamic)
      for (int i = 1; i < nbVilles; i++)
      {
        if (!present(i, mask))
        {
          ...
          tmp_chemin[etape] = i;
          dist = distance[ici][i];
          tsp_ompfor(etape + 1, lg + dist, tmp_chemin, mask | (1 << i));
        }
      }
    }
  }
...
// tsp_task() with firstprivate
void tsp_task_fistprivate(int etape, int lg, chemin_t chemin, int mask)
{
  int ici, dist;
  if (lg + distance[0][chemin[etape-1]]>= minimum) 
    return;
...
int main(int argc, char **argv)
{
  ...
  else if (!strcmp(argv[argc-1],"ompfor4")) 
    tsp_ompcol4();
  else if (!strcmp(argv[argc-1],"ompfor3")) 
    tsp_ompcol3();
  else if (!strcmp(argv[argc-1],"ompfor2")) 
    tsp_ompcol2();
  ...
  else if (!strcmp(argv[argc-1],"task-first")){
    #pragma omp parallel
    #pragma omp single
    tsp_task_fistprivate(1, 0, chemin, 1);
  }
  else if (!strcmp(argv[argc-1],"task-dynamic")){
    #pragma omp parallel
    #pragma omp single
    tsp_task_dynamic(1, 0, chemin, 1);
  }
  else
  {
