...
static __m256 _mm256_atan_ps (__m256 x)
{
  ...
  __m256 one = _mm256_set1_ps(1.0); // vecteur 1 1 1 1 1 1 1 
   __m256 pi4 = _mm256_set1_ps(M_PI_4);
  __m256 t = _mm256_sub_ps(one, _mm256_abs_ps(x)); // 
  t = _mm256_mul_ps(t,x);
  t = _mm256_mul_ps(t, _mm256_set1_ps(0.273));
  return _mm256_fmadd_ps(x,pi4,t);
...
static __m256 _mm256_atan2_ps (__m256 y, __m256 x)
{
  ...
  th = _mm256_add_ps(th, _mm256_and_ps(mask,pi2));

  mask = _mm256_cmp_ps(x,zero,_CMP_LT_OS);

 th = _mm256_negmask_ps(th,mask);
 th = _mm256_add_ps(th, _mm256_and_ps(mask,pi));

 mask = _mm256_cmp_ps(y,zero,_CMP_LT_OS);
